...
RSpec.describe "Api::V1::Consultations", type: :request do
  ...
  describe "POST /create" do
    it "creates consultation with valid consultation" do
      ...
      post "/api/v1/consultations/create", params: { consultation: valid_consultation }, headers: set_headers
    ...
    it "rejects consultation creation with invalid consultation" do
      login_and_confirm(user)
      post "/api/v1/consultations/create", params: { consultation: invalid_consultation }, headers: set_headers
  ...
  describe "PATCH /update" do
    it "updates consultation with valid consultation" do
      login_and_confirm(user)
      patch "/api/v1/consultations/update/#{consultation.uid}", params: { consultation: valid_consultation }, headers: set_headers
    ...
    it "rejects consultation update with invalid consultation" do
      login_and_confirm(user)
      patch "/api/v1/consultations/update/#{consultation.uid}", params: { consultation: invalid_consultation }, headers: set_headers
      expect(json_response["errors"].size).to be > 0
      expect(response).to have_http_status(:unprocessable_entity)
    end
  end
  ...
  describe "DELETE /destroy" do
    it "removes consultation from user" do
      login_and_confirm(user)
      delete "/api/v1/consultations/destroy/#{consultation.uid}", headers: set_headers
