...
RSpec.describe "Api::V1::Users", type: :request do
  ...
  describe "POST /update_email" do
    it "sends a confirmation email with valid email" do
      ...
      post "/api/v1/users/update-email", params: { user: valid_email }, headers: set_headers
    ...
    it "rejects email update with invalid email" do
      login_and_confirm(user)
      post "/api/v1/users/update-email", params: { user: invalid_email }, headers: set_headers
  ...
  describe "POST /update_password" do
    it "updates password with valid password" do
      login_and_confirm(user)
      post "/api/v1/users/update-password", params: { user: valid_password }, headers: set_headers
    ...
    it "rejects password update with invalid password" do
      login_and_confirm(user)
      post "/api/v1/users/update-password", params: { user: invalid_password }, headers: set_headers
      expect(json_response["errors"].size).to be > 0
      expect(response).to have_http_status(:unprocessable_entity)
    end
  end
  ...
  describe "DELETE /destroy" do
    it "removes user from the database" do
      login_and_confirm(user)
      delete "/api/v1/users/destroy", headers: set_headers
