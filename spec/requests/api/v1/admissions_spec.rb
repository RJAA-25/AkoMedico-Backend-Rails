...
RSpec.describe "Api::V1::Admissions", type: :request do
  ...
  describe "POST /create" do
    it "creates admission with valid admission" do
      ...
      post "/api/v1/admissions/create", params: { admission: valid_admission }, headers: set_headers(user)
    ...
    it "rejects admission creation with invalid admission" do
      login_and_confirm(user)
      post "/api/v1/admissions/create", params: { admission: invalid_admission }, headers: set_headers(user)
  ...
  #     login_and_confirm(user)
  #     patch "/api/v1/admissions/update/#{admission.uid}", params: { admission: valid_admission }, headers: set_headers(user)
  ...
  #     login_and_confirm(user)
  #     patch "/api/v1/admissions/update/#{admission.uid}", params: { admission: invalid_admission }, headers: set_headers(user)
  #     expect(json_response["errors"].size).to be > 0
  #     expect(response).to have_http_status(:unprocessable_entity)
  #   end
  # end
  ...
  #     login_and_confirm(user)
  #     delete "/api/v1/admissions/destroy/#{admission.uid}", headers: set_headers(user)
